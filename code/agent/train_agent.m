...
% Trains ACTOR on the panoptic environment
...
stats.register('avg', 'Hip-aligned error RL');
...
function [snapshot_results, pathname, snapshot_errs_vs_steps, ...
    ...
    baselines = BaselinesRunner(CONFIG.training_baselines);
    ...
    if CONFIG.evaluation_init_snapshot
        ...
        err_std_all_hip_aligned = cell(1, numel(CONFIG.evaluation_nbr_actions));
        err_std_bls_all = cell(1, numel(CONFIG.evaluation_nbr_actions));
        err_std_bls_all_hip_aligned = cell(1, numel(CONFIG.evaluation_nbr_actions));
        ...
        for i = 1 : numel(CONFIG.evaluation_nbr_actions)
            ...
            err_std_all_hip_aligned{i} = eval_out.err_std_hip_aligned;
            err_std_bls_all{i} = eval_out.err_std_bls;
            err_std_bls_all_hip_aligned{i} = eval_out.err_std_bls_hip_aligned;
        ...
        snapshot_results{end + 1} = {pathname, err_std_all, err_std_bls_all, ...
                                     ...
                                     avg_traj_lens_all, err_std_all_hip_aligned, ...
                                     err_std_bls_all_hip_aligned, err_std_bls_all_traj_len}; %#ok<*AGROW>
    end
    ...
    for ep = 1 : CONFIG.training_agent_nbr_eps
        ...
        if CONFIG.predictor_limited_caching
            error('Limited caching not supported during training');
        end
        ...
        % Run agent-environment interaction (i.e. let ACTOR generate an
        % active-sequence in Panoptic)
        timer.tic('rl_forward');
        [out_sequence, recorder] = ...
        ...
        % Run baselines
        timer.tic('Baselines');
		bl_errors = baselines.run(env, out_sequence);
        for i = 1 : size(bl_errors, 1) % Collect training statistics
            for j = 1 : size(bl_errors, 2)
                stats.s(bl_errors{i}.name).collect([bl_errors{i, j}.error, ...
                        bl_errors{i, j}.error_hip_aligned, ...
                        numel(bl_errors{i, j}.visited_cams) - 1]);
            end
        end
        timer.toc('Baselines');

        % Perform train step (update policy network parameters)
        ...
        if ep < CONFIG.training_agent_nbr_eps && rem(ep, CONFIG.training_agent_snapshot_ep) == 0
			...
			snapshot_results{end + 1} = {pathname, err_std_all, err_std_bls_all, ...
										 ...
										 avg_traj_lens_all, err_std_all_hip_aligned, ...
										 err_std_bls_all_hip_aligned, err_std_bls_all_traj_len};
...
function eval_out = run_evaluation(env, net, weight_path, data_names, ...
    ...
    % Setup RL agent
    agent = ACTOR(net, nan, CONFIG.training_agent_eps_per_batch, ...
    ...
    eval_stats.register('avg', 'Hip-aligned error RL');
    ...
    % Stop timer
    end_time = toc(start_time_eval);
    fprintf('Evaluation on validation set done! Elapsed time: %.2f sec\n\n', end_time);
    ...
    eval_out.err_std_hip_aligned = curr_err_std.hip_aligned;
    eval_out.err_std_bls = curr_err_std_bl.errs;
    eval_out.err_std_bls_hip_aligned = curr_err_std_bl.hip_aligned;
    eval_out.err_std_all = curr_err_std.errs_all;
    eval_out.err_std_hip_aligned_all = curr_err_std.hip_aligned_all;
    eval_out.err_std_bls_all = curr_err_std_bl.errs_all;
    eval_out.err_std_bls_hip_aligned_all = curr_err_std_bl.hip_aligned_all;
...
function evaluate_agent_on_validation_set(env_val, net_val, save_path, ...
    ...
    orig_stop_tri = CONFIG.agent_stop_triangulated;
    ...
    err_std_all_hip_aligned_all = cell(1, numel(CONFIG.evaluation_nbr_actions));
    err_std_bls_all_all = cell(1, numel(CONFIG.evaluation_nbr_actions));
    err_std_bls_all_hip_aligned_all = cell(1, numel(CONFIG.evaluation_nbr_actions));
    err_std_bls_all_traj_len = cell(1, numel(CONFIG.evaluation_nbr_actions));
    err_std_scenes_all = cell(1, numel(CONFIG.evaluation_nbr_actions));
    err_std_bls_scenes_all = cell(1, numel(CONFIG.evaluation_nbr_actions));
    avg_traj_lens_all = cell(1, numel(CONFIG.evaluation_nbr_actions));
    ...
    for i = 1 : numel(CONFIG.evaluation_nbr_actions)
        ...
        err_std_all_hip_aligned_all{i} = eval_out.err_std_hip_aligned_all;
        err_std_bls_all_all{i} = eval_out.err_std_bls_all;
        err_std_bls_all_hip_aligned_all{i} = eval_out.err_std_bls_hip_aligned_all;
		...
		snapshot_results{end} = {pathname, err_std_all, err_std_bls_all, ...
								 ...
								 avg_traj_lens_all, err_std_all_hip_aligned, ...
								 err_std_bls_all_hip_aligned, err_std_bls_all_traj_len, ...
                                 err_std_all_all, err_std_bls_all_all, err_std_all_hip_aligned_all, ...
                                 err_std_bls_all_hip_aligned_all};
		save(strcat(CONFIG.output_dir, 'snapshot_results.mat'), 'snapshot_results');
    end
    CONFIG.evaluation_nbr_actions = orig_nbr_actions;
    CONFIG.agent_stop_triangulated = orig_stop_tri;
...
function print_snapshot_results(snapshot_results, training_stats, tmp_nbr_actions)
    ...
    y_agent_hip_aligned = nan(size(y_agent));
    y_baselines_init = nan(size(y_baselines));
    y_baselines_hip_aligned = nan(size(y_baselines));
    ...
    for i = 1 : numel(snapshot_results)
        ...
        for nbr_fix_idx = 1 : numel(CONFIG.evaluation_nbr_actions)
            ...
            err_std_hip_aligned = result{8}{nbr_fix_idx};
            bls_hip_aligned = result{9}{nbr_fix_idx};
            bls_traj_len = result{10}{nbr_fix_idx};
            if nbr_fix_idx == 1
            ...
            fprintf('Corresponding hip-aligned results:\n');
            y_agent_hip_aligned(nbr_fix_idx, i) = err_std_hip_aligned(1);
            fprintf('Agent -- mean error: %.3f\n', err_std_hip_aligned(1));
            for j = 1 : numel(CONFIG.evaluation_baselines)
                fprintf('Baseline %s -- mean error: %.3f\n', ...
                        CONFIG.evaluation_baselines{j}, bls_hip_aligned(1, j));
                y_baselines_hip_aligned(nbr_fix_idx, j, i) = bls_hip_aligned(1, j);
            end
            fprintf('\n');
        ...
        if i == numel(snapshot_results) && CONFIG.evaluation_compute_all_fixations
            ...
            % number of fixations
            err_std_all = result{end - 3};
            err_std_bl_all = result{end - 2};
            err_std_hip_aligned_all = result{end - 1};
            err_std_bl_hip_aligned_all = result{end};
    ...
    % Same for hip-aligned
    for nbr_fix_idx = 1 : numel(CONFIG.evaluation_nbr_actions)
        nbr_actions = CONFIG.evaluation_nbr_actions(nbr_fix_idx);
        y_tmp = y_agent_hip_aligned(nbr_fix_idx, :);
        series = helpers.hyperdock_serie('Agent', x, y_tmp(:)');
        for j = 1 : numel(CONFIG.evaluation_baselines)
            data = y_baselines_hip_aligned(nbr_fix_idx, j, :);
            series = [series, helpers.hyperdock_serie(CONFIG.evaluation_baselines{j}, x, data(:)')];
        end
        plot_name = sprintf('Hip-Aligned Recon Error (%d views)', nbr_actions);
        plots = [plots, helpers.hyperdock_plot(plot_name, 'Episodes', 'Error', series)];
    end
    ...
    if ~any(isnan(mean_traj_length))
        ...
        if CONFIG.baselines_stop_triangulated
            for j = 1 : numel(CONFIG.evaluation_baselines)
                data = y_baselines_traj_len(1, j, :);
                series = [series, helpers.hyperdock_serie(CONFIG.evaluation_baselines{j}, x, data(:)')];
            end
        end
